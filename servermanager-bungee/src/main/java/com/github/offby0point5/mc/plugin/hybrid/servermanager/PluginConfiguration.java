    ...
    private static class RestHost {
        ...
        private RestHost(CommentedConfig config) {
            ...
            try {
                ...
                ServermanagerBungee.logger.log(Level.SEVERE, "'bind' option does not specify a valid IP address.", e);
            }
        }
    }
    ...
    private static class Servers {
        private Servers(CommentedConfig config) {
            ...
            for (UnmodifiableConfig.Entry entry : config.entrySet()) {
                if (ServermanagerBungee.proxy.getServerInfo(entry.getKey()) == null) continue;
